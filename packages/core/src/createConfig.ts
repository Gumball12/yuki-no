...
export const createConfig = (): Config => {
  ...
  // Compatibility layer: automatically add plugin when release-tracking is enabled
  const finalPlugins = [...plugins];
  const releaseTrackingPkg = '@gumball12/yuki-no-plugin-release-tracking';
  if (releaseTracking && !finalPlugins.includes(releaseTrackingPkg)) {
    finalPlugins.push(releaseTrackingPkg);
  }
