...
describe('createPrBody', () => {
  describe('when given empty issue status array', () => {
    test('should return PR body with empty resolved issues section', () => {
      ...
      // Resolved section should not contain any issue references
      const resolvedSection = result
        .split('### Resolved Issues')[1]
        .split('---')[0];
      expect(resolvedSection).not.toMatch(/Resolved #\d+/);
      // And should include the Excluded Files section header
  ...
  describe('PR body structure validation', () => {
    ...
    test('should have proper markdown formatting', () => {
      ...
      expect(result).toContain(
        '[Yuki-no Batch PR Plugin](https://github.com/Gumball12/yuki-no/tree/next/packages/batch-pr/)',
      );
    });
  });
  ...
  describe('excluded files section', () => {
    test('should render excluded section header even when empty', () => {
      const body = createPrBody([{ number: 1, type: 'Resolved' }], {
        excludedFiles: [],
      });
      expect(body).toContain('### Excluded Files (manual changes required)');
    });

    test('should render excluded section with file list when provided', () => {
      const body = createPrBody([{ number: 1, type: 'Resolved' }], {
        excludedFiles: ['docs/README.md', 'temp/file.tmp'],
      });
      expect(body).toContain('### Excluded Files (manual changes required)');
      expect(body).toContain('YUKI_NO_BATCH_PR_EXCLUDE');
      expect(body).toContain('`docs/README.md`');
      expect(body).toContain('`temp/file.tmp`');

      // Section should be placed before the horizontal rule (---), and under Resolved Issues
      const [beforeHr] = body.split('---');
      expect(beforeHr).toContain('### Resolved Issues');
      const resolvedSection = beforeHr.split('### Resolved Issues')[1];
      expect(resolvedSection).toContain('### Excluded Files');
    });
  });
});
