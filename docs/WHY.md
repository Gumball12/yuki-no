...
_Example: The `@gumball12/yuki-no-plugin-release-tracking` plugin automatically manages release status_
...
For instance, the built-in `@gumball12/yuki-no-plugin-release-tracking` plugin provides automated release status tracking using Issue Comments and Labels for projects where release management is critical. This plugin-based approach allows teams to build exactly the translation management system they need, whether simple or complex, while maintaining compatibility and ease of use.
